...
const promise = async () => {
    ...
    const {UTIL: PATIENT_UTIL} = await import(`/js/custom/constant/patient/util.js${ver_string}`);
    ...
    const html = await import(`/template/${GBL.DESIGN.THEME}/content/patient.js${ver_string}`);
    const htmlForUnit = await import(`/template/${GBL.DESIGN.THEME}/content/unit.js${ver_string}`);
    // await import(`/js/custom/constant/patient/data-init.js${ver_string}`);
    ...
    const _search = function(type = `search`) { //first, search
        const params = {
            ...
            paging: {
                ...
                domSelector: `.patient-pagination`
            },
            searchAddParams: {
                'patientTypeList': Number($(`input[name=patient-type]:checked`).val()) === -1 ? null : [Number($(`input[name=patient-type]:checked`).val())],
                'transportStatusList': Number($(`input[name=transport-status]:checked`).val()) === -1 ? null : [Number($(`input[name=transport-status]:checked`).val())],
            },
            useFunction: {
                parsing: PATIENT_UTIL.DATA_PARSING,
                search: PATIENT_UTIL.LIST,
                afterEvent: setAddEvent, // 파라미터로는 stirng > `datas` 가 전달된다.
                paging: _search
            },
            data: {
                domSelector: PATIENT_CONST.DESIGN.CONTENTS_BY_DATA_TABLE,
                resultKey: `brainSaverPatientList`
            },
            constains: {
                PATIENT_TYPE: Array.deepCopy(PATIENT_CONST.PATIENT_TYPE)
            },
    ...
    const setAddEvent = function(content = `index`){
        ...
        if (content === `index`) {
            ...
            $(".button-excel").off("click").on("click", function (e) {
                etc.stopBubbling(e);
                const currentPage = $(`#patient_currentPage`).length > 0 ? Number($(`#patient_currentPage`).val()) : 1;
                const params = {
            ...
            $(`.button-update`).off("click").on("click", function (e) {
                etc.stopBubbling(e);
                etc.move(`/patient/update?measurementCode=${$(this).closest('.cm-tr').attr('data-unique-id')}`);
            });

            CUSTOM.EVENT.HTML.push(".button-update-hospital");
            $(".button-update-hospital").off("click").click(function (e) {
                etc.stopBubbling(e);
                PATIENT_UTIL.HOSPITAL_CHANGE($(this).closest('.cm-tr').attr('data-unique-id'), html.modalForHospitalChange, _search);
            });

            CUSTOM.EVENT.HTML.push(".button-update-transport");
            $(".button-update-transport").off("click").click(function (e) {
                etc.stopBubbling(e);
                PATIENT_UTIL.TRANSPORT_CHANGE($(this).closest('.cm-tr').attr('data-unique-id'), html.modalForTransportChange, _search);
            });

            CUSTOM.EVENT.HTML.push(`.button-delete`)
            $(`.button-delete`).off("click").on("click", function (e) {
                etc.stopBubbling(e);
                const modalId = "customAlertPatientDelete";
                const selectedUniqueId = $(this).closest('.cm-tr').attr('data-unique-id');
                const okBtnCallback = function () {
                    const response = PATIENT_UTIL.DELETE(selectedUniqueId);
            ...
            $(".btn-confirm").off("click").on("click", function (e) {
                ...
                if(etc.formCheck(dataFormId, _addCheck, `error`) === true) {
                    ...
                    const _t = PATIENT_UTIL.INSERT(params);
                    if (_t.result === true) {
                        let processEnd = function(mode = `list`) { //list, add
                            if (mode === `list`) {
                                etc.move(`/patient/index`);
                            }
                            else {
                                location.reload();
                            }
            ...
            $(".btn-confirm").off("click").on("click", function (e) {
                ...
                if(etc.formCheck(dataFormId, null, `error`) === true) {
                    ...
                    const params = {
                        updateType: PATIENT_CONST.UPDATE_TYPE.CODE.CODE0,
                        patientType : form['patient-type'],
                        // latitude : form['latitude'],
                        // longitude : form['longitude'],
                    ...
                    const _t = PATIENT_UTIL.UPDATE(choiceMeasurementCode, params);
                    if (_t.result === true) {
                        let processEnd = function(mode = `list`) { //list, add
                            if (mode === `list`) {
                                etc.move(`/patient/index`);
    ...
    const index = function() {
        const _c = {
            PATIENT_TYPE: Array.deepCopy(PATIENT_CONST.PATIENT_TYPE.TITLE),
            TRANSPORT_STATUS: Array.deepCopy(PATIENT_CONST.TRANSPORT_STATUS.TITLE),
            // IS_TRANSFER: Array.deepCopy(PATIENT_CONST.IS_TRANSFER.TITLE),
        }
        const params = {
            choiceSearchOptions: {
                patientType: [-1],
        ...
        if (GBL.CONSTANTS.has(`SEARCH.OPTIONS`) === true) {
            ...
            if (_s.patientTypeList !== null) {
                params.choiceSearchOptions.patientType = _s.patientTypeList;
            }
    ...
    const add = function() {
        let DIAGNOSIS_TYPE2 = Array.deepCopy(PATIENT_CONST.DIAGNOSIS_TYPE.TITLE);
        delete DIAGNOSIS_TYPE2[PATIENT_CONST.DIAGNOSIS_TYPE.CODE.NOT];
        const _c = {
            TRANSPORT_STATUS: Array.deepCopy(PATIENT_CONST.TRANSPORT_STATUS.TITLE),
            PATIENT_TYPE: Array.deepCopy(PATIENT_CONST.PATIENT_TYPE.TITLE),
            AGE_RANGE: Array.deepCopy(PATIENT_CONST.AGE_RANGE.TITLE),
            DIAGNOSIS_TYPE: Array.deepCopy(PATIENT_CONST.DIAGNOSIS_TYPE.TITLE),
    ...
    const update = function(params = null) {
        ...
        const _t = PATIENT_UTIL.INFO(params.measurementCode);
        if (_t.result === true) {
            _t.brainSaverPatient = PATIENT_UTIL.DATA_PARSING(_t.brainSaverPatient);
            $(`${GBL.DESIGN.PAGE_DIV_NAME}`).html("");
            etc.setHtmlParsing($(`${GBL.DESIGN.PAGE_DIV_NAME}`), html.update, {title: `수정`, brainSaverPatient: _t.brainSaverPatient});
