...
Seers.Loader = {
    _controllerLoad: async function(controller, action, id, parameters = null) {
    ...
    directControllerLoad: async function(controller, action) {
        showSiteLoadingLog(`loader > direct controllerLoad parent`, `#1ed7f8`, `#d9e6f1`);
        return await Seers.Loader._controllerLoad(controller, action);
    },

    controllerLoad: async function () {
        ...
        return await Seers.Loader._controllerLoad(controller, action, id, parameters);
    },
    ...
    moduleLoad: async function (module = "alert", action = "index", parameter = null) {
        ...
        async function moduleLoad(module, action, parameter) {
            ...
            //
            //         if (typeof GBL.MODULE.IS_PRE_LOADING[module] == "undefined") {
            //             console.log("module.obj.hasOwnProperty(\"pre\") === true::::::::::::", obj.hasOwnProperty("pre"));
            //             if (obj.hasOwnProperty("pre") === true) {
            //                 GBL.MODULE.IS_PRE_LOADING[module] = await obj.pre();
            //             } else {
            //                 obj.pre = function () {
            ...
            //         // etc.hrefToSeersLink();
            //
            //         console.log(`MODULE ${module} is LOAD OK`);
            //         return true;
            //     }
            // } catch (error) {
            //     if(error.message.indexOf("module:") !== -1) {
    ...
    goMove: async function(title = "seers", ctl = "login", action = null, id = null, parameter = null, fullUrl = null) {
        let link = "";
        link += "/" + ctl;
        if(action != null) {
            link += "/" + action;
        ...
        return await Seers.Loader.controllerLoad();
    }
}