...
let etc = {
     ...
     * @returns {void} 없음
     */
    move: function(linkValue = null) {
        // 같은 주소가 다시 로딩되지 않게
        if (GBL.WINDOW_HISTORY_STATE.URL[0] !== linkValue) {
            let e = window.event;
            ...
            Seers.Loader.goMove(title, controller, action, id, String.getUrlParam(linkValue), linkValue);
            return;
        }
    },
    ...
    validate: {
        birthday: function(value) {
            // 시리얼 번호
            let alertMsg = null;
            let _pattern = /^(19[0-9][0-9]|20\d{2})-(0[0-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/
